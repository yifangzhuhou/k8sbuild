...
这里我再介绍下node如何加入到集群中。首先需要向master提供预先给定的token和sha256，我们在进入到master容器内，执行下面命令获取
token和sha256。
```
...
然后我们可以在master上通过执行`kubectl get nodes`或访问dashboard来查看集群中各节点状态了，以下是结果
...
![master.png](./static/master.png)

![slave.png](./static/slave.png)
